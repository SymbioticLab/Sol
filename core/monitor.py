...
def log(*s):
    ...
    today = datetime.now()
    with open("/proj/gaia-PG0/HSpark/spark/bench/report/monitor_proc.log", 'a') as f:
        f.write( str(today.strftime(' %a %d-%m-%Y @ %H:%M:%S')) + ": " + log_str )
...
def sig_term_handler(signo, stack):
    ...
    na.stop()
    generate_report(workload_title, log_path, bench_log_path, report_path, nodes_to_monitor)
    sys.exit(0)
...
import time, os, sys, socket, traceback
def log(x, **kw):
  with open("/users/fanlai/spark/spark/hibench/util_{randInt}.log", kw.get("mode","a")) as f:
    f.write(x)

with open("/users/fanlai/spark/spark/hibench/util_{randInt}.log", "w") as f:
  pass
s=socket.socket(socket.AF_INET, socket.SOCK_STREAM)
s.bind(("0.0.0.0",0))
s.listen(1)
...
while True:
  _time = time.time()
  info = ("{SEP}+%s" % _time)+chr(10)+"****"
{call_template}
  info = info + "{SEP}#end"+chr(10)+"****"
  log(info, mode="a")
  time.sleep({interval})
')"""
    ...
    def __init__(self, host, interval=1):
        ...
        self._running=True
        self.rand_seed = random.randint(0, 100)
...
#            log("ssh client %s connected" % self.host)
            ...
            with closing(s.makefile()) as f2:
                log("entering the s.makefile()....")
                while self._running:
                    pass
                    
            log("entering the shutdown ssh....")
            s.shutdown(socket.SHUT_RDWR)
        self.ssh_close()

    def getResults(self, sampleCounts):
        cnt = 0

        try:
            with open("/users/fanlai/spark/spark/results/"+str(self.host)+".log") as fin:
                lines = fin.readlines()
                script = "".join(x for x in lines)
                items = script.split("****")

                container=[]
                
                for i, l in enumerate(items):
                    #log("start loops..." + str(i))
                    if l.startswith(self.SEP):
                        tail = l.lstrip(self.SEP)
                        if tail[0]=='+': # timestamp
                            remote_timestamp = float(tail[1:])
                            cur_timestamp = remote_timestamp
                        elif tail.startswith('#end'): # end sign
                            self.na_push(cur_timestamp)
                            cnt += 1
                            if cnt == sampleCounts:
                                log("Done getResults...." + str(cur_timestamp))
                                return
                        else:
                            id = int(tail[1:])
                        ...
                        iis = l.strip().split('\n')
                        for ii in iis:
                            container.append(ii.strip())
        except Exception as e:
            log(str(e))

    def sampleResults(self):
        cmd = "scp {host}:/users/fanlai/spark/spark/hibench/util_{randInt}.log /users/fanlai/spark/spark/results/{host}.log".format(host=self.host,randInt=self.rand_seed)
        os.system(cmd)
...
class BaseMonitor(object):
    ...
    def commit(self, timestamp, header, stat, ignore=["lo"]):
...
class CPUMonitor(BaseMonitor):
  ...
  info = "".join([x for x in f.readlines() if x.startswith("cpu")])
""")
...
class NetworkMonitor(BaseMonitor):
  ...
  info = "".join([x for x in f.readlines()])
""")
...
class DiskMonitor(BaseMonitor):
  ...
  info = "".join([x for x in f.readlines() if x.split()[2] in blocks and not x.split()[2].startswith("loop") and x.split()[3]!="0"])
""")
...
class MemoryMonitor(BaseMonitor):
  ...
  info = ":".join([mem[field] for field in ["MemTotal", "Buffers", "Cached", "MemFree", "Mapped"]])+chr(10)
""")
...
class NodeAggregator(object):
    ...
    def commit_aggregate(self, node, datas):
        ...
        with self.log_lock:
            with file(self.log_name, "a") as f:
                #f.write(str(today.strftime(' %a %d-%m-%Y @ %H:%M:%S')) + ':' + repr(datas) + "\n")
                f.write(repr(datas) + "\n")
    ...
    def stop(self):
        ...
        trunction = 2147483647
        for v in self.node_pool.values():
            sampleCounts = v.sampleResults()
            trunction = min(sampleCounts, trunction)
...
def generate_report(workload_title, log_fn, benchlog_fn, report_fn, nodes):
    log("stepping in generate report")
    c =- 1
    ...
    #log("Network Overall", repr(network_overall))
    #log("CPU Overall", repr(cpu_overall))
...
if __name__=="__main__":
    ...
    with open("/proj/gaia-PG0/HSpark/spark/bench/report/monitor_proc.log", 'w') as f:
        pass
