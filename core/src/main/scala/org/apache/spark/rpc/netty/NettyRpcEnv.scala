...
private[netty] class NettyRpcEnv(
  ...
  override def setupEndpoint(name: String, endpoint: RpcEndpoint): RpcEndpointRef = {
    val rpcRef = dispatcher.registerRpcEndpoint(name, endpoint)
    logInfo(s"====setupEndpoint with rpcRef address ${rpcRef.address}")
    rpcRef
  }
...
private[rpc] class NettyRpcEnvFactory extends RpcEnvFactory with Logging {
  ...
  def create(config: RpcEnvConfig): RpcEnv = {
    ...
    if (!config.clientMode) {
      logInfo(s"====try to startNettyRpcEnv port ....")
      val startNettyRpcEnv: Int => (NettyRpcEnv, Int) = { actualPort =>
        nettyEnv.startServer(config.bindAddress, actualPort)
        logInfo(s"====Actually bind to ${nettyEnv.address}, and actualPort is ${actualPort}")
        (nettyEnv, nettyEnv.address.port)
      }
      try {
        logInfo(s"====try to create listen port ....")
        Utils.startServiceOnPort(config.port, startNettyRpcEnv, sparkConf, config.name)._1
        logInfo(s"====Successfully creating listen port ....${config.port}")
      } catch {
        case NonFatal(e) =>
          nettyEnv.shutdown()
          throw e
