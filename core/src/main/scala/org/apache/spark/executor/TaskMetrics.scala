...
class TaskMetrics private[spark] () extends Serializable with Logging {
  ...
  private val _executorQueuingTime = new LongAccumulator
  private var _shuffleStartTime = new AtomicLong(0)
  private var _shuffleCompleteTime = new AtomicLong(0)

  /**
   * Time spent on queuing in the pipeline containers.
   */
  def executorQueuingTime: Long = _executorQueuingTime.sum
  ...
  private[spark] def setShuffleTimeStart(v: Long): Unit = {
    if (_shuffleStartTime.get == 0 || v < _shuffleStartTime.get) {
      _shuffleStartTime.set(v)
    }
  }

  private[spark] def setShuffleTimeComplete(v: Long): Unit = {
    if (_shuffleCompleteTime.get < v) {
      _shuffleCompleteTime.set(v)
    }
  }

  private[spark] def setShuffleBlockTime(): Unit = {
    shuffleReadMetrics.setFetchWaitTime(_shuffleCompleteTime.get - _shuffleStartTime.get)
  }

  private[spark] def setExecutorQueuingTime(v: Long): Unit = 
    _executorQueuingTime.setValue(v)
  ...
  @transient private[spark] lazy val nameToAccums = LinkedHashMap(
    EXECUTOR_QUEUING_TIME -> _executorQueuingTime,
