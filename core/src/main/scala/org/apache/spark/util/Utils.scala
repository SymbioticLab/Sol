...
private[spark] object Utils extends Logging {
  ...
  def doFetchFile(
    ...
    logInfo("========= doFectchFile: " + Option(uri.getScheme).getOrElse("file") + " :" + targetDir.toString + " " + filename)
    val startTime = System.currentTimeMillis()

    // check whether the jars are available locally or not
    var doneFetch = false
    if (conf.getBoolean("spark.localjar.enable", false)){
      try {
        val rootDir = (targetDir.toString).split("spark-")(0)
        val jarName = url.split("/").last

        if (Files.exists(Paths.get(rootDir + jarName))){
          copyFile(url, new File(rootDir + jarName), targetFile, fileOverwrite)
          doneFetch = true
          logInfo("====== Successful read from local jars: " + jarName)
        }
      } catch{
      case e: Exception => 
        logInfo("Failed to read from local")
        e.printStackTrace()
      } finally {
        if (!doneFetch)
          logInfo("====== Failed to read from local: " + url)
      }
    }   
    ...
    logInfo ("====== Time on downloading jars: " + (System.currentTimeMillis() - startTime).toString)
    targetFile
  }
