...
class SparkContext(config: SparkConf) extends Logging {
  ...
  val addedReader = new ConcurrentHashMap[String, RecordReader[Any, Any]]().asScala
  ...
  def broadcast[T: ClassTag](value: T): Broadcast[T] = {
    ...
    logInfo("Created broadcast " + bc.id + " from " + callSite.shortForm + ", start from " + System.currentTimeMillis.toString)
  ...
  def stop(): Unit = {
    ...
    if (!conf.getBoolean("spark.httpbroadcast.enabled", false)) {
      Utils.tryLogNonFatalError {
