      ...
      <svg>
        {
          val legendPairs = List(("scheduler-delay-proportion", "Scheduler Delay"),
            ("queuing-delay-proportion", "Queuing Delay"),
  ...
  def makeTimeline(tasks: Seq[TaskData], currentTime: Long): Seq[Node] = {
    ...
    val executorsArrayStr =
      tasks.sortBy(-_.launchTime.getTime()).take(MAX_TIMELINE_TASKS).map { taskInfo =>
        ...
        val queuingDelay = metricsOpt.map(_.executorQueuingTime).getOrElse(0L)
        val queuingDelayProportion = toProportion(queuingDelay)
        ...
        val otherTime = totalExecutionTime - schedulerDelay - queuingDelay - shuffleReadTime - gettingResultTime
        ...
        val svgTag =
          if (totalExecutionTime == 0) {
           ...
           s"""<svg class="task-assignment-timeline-duration-bar">
                 ...
                 |<rect class="queuing-delay-proportion"
                   |x="$queuingDelayProportionPos%" y="0px" height="26px"
                   |width="$queuingDelayProportion%"></rect>
          ...
          s"""
             |{
               ...
               |'content': '<div class="task-assignment-timeline-content"
                 ...
                 |<br>Task Queuing Time: ${UIUtils.formatDuration(queuingDelay)}
