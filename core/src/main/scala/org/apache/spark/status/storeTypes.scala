...
private[spark] class TaskDataWrapper(
    ...
    @KVIndexParam(value = TaskIndexNames.QUEUING_TIME, parent = TaskIndexNames.STAGE)
    val executorQueuingTime: Long) {
  ...
  def schedulerDelay: Long = {
    if (hasMetrics) {
      AppStatusUtils.schedulerDelay(launchTime, resultFetchStart, duration, executorDeserializeTime,
        resultSerializationTime, executorRunTime, executorQueuingTime)
    } else {
      -1L
    }
  }
...
private[spark] class CachedQuantile(
    ...
    val executorQueuingTime: Double) {
