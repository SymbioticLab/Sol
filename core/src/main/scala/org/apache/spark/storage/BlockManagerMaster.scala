...
class BlockManagerMaster(
  ...
  def getBlock(blockId: BlockId): ManagedBuffer = {
    logInfo(s"====Send request for block ${blockId.name}")
    val msg = driverEndpoint.askSync[Array[Byte]](GetBlock(blockId))
    val data = new NioManagedBuffer(ByteBuffer.wrap(msg))
    logInfo(s"====Compalete request for block ${blockId.name}")
    data
  }
