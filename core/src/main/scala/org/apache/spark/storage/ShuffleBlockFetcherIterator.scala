...
final class ShuffleBlockFetcherIterator(
  ...
  private[this] val convertToLocal = new HashSet[BlockId]
  ...
  private[this] val nextCount = new AtomicInteger(0)

  shuffleMetrics.incFetchWaitTime(getInfoTime)

  logInfo(s"====shuffleMetrics.incFetchWaitTime ${getInfoTime} ms")
  ...
  private[this] def cleanup() {
    ...
    while (iter.hasNext) {
      ...
      result match {
        case SuccessFetchResult(_, address, _, buf, _) =>
          ...
          logInfo(s"====Start to cleanup now with size ${buf.size}")
          buf.release()
        case _ =>
      }
    }
    ...
    if (autoClean) {
      convertToLocal.map {
        case blockId =>
          // try to remove from the local store 
           blockManager.tryToRemoveBlock(blockId)
      }
    }
  }
  ...
  private[this] def sendRequest(req: FetchRequest) {
    ...
    val blockFetchingListener = new BlockFetchingListener {
      override def onBlockFetchSuccess(blockId: String, buf: ManagedBuffer): Unit = {
        ...
        logTrace("====Got remote block " + blockId + " after " + Utils.getUsedTimeMs(startTime) + " from " + address)
      }
  ...
  private[this] def splitLocalRemoteBlocks(): ArrayBuffer[FetchRequest] = {
    ...
    for ((address, blockInfos) <- blocksByAddress) {
      if (address.executorId == blockManager.blockManagerId.executorId) {
        ...
        localBlocks ++= blockInfos.map{
            case blockInfo => 
                // convert to local
                if (blockInfo._1.isConverted == 1) {
                  convertToLocal += blockInfo._1
                } else if (blockInfo._1.isConverted == 2)
                  prePostBlocks += 1
    ...
    logTrace(s"Getting $numBlocksToFetch non-empty blocks including ${localBlocks.size}" +
        s" local blocks and ${remoteBlocks.size} remote blocks, ${prePostBlocks} prePostBlocks")
    remoteRequests
  }
  ...
  private[this] def fetchLocalBlocks() {
    ...
    while (iter.hasNext) {
      ...
      //logInfo(s"====Start local fetch for block:${blockId.name}")
      try {
        val inMemoryFetch = convertToLocal.contains(blockId) match {
            case true => 
              if (blockManager.cachedInMemory(blockId)) {
                2
              }
              else
                1
        ...
        val buf = inMemoryFetch match {
          case 2 =>
            new NettyManagedBuffer(blockManager.getConvertBlockDataFromMemory(blockId).toNetty)
          case 1 => 
              blockManager.getConvertBlockData(blockId)
          case _ => 
  ...
  override def next(): (BlockId, InputStream) = {
    ...
    while (result == null) {
      ...
      val duration = stopFetchWait - startFetchWait
      //logInfo(s"====To fetch ${result.blockId} takes ${duration} ms")
      
      shuffleMetrics.incFetchWaitTime(stopFetchWait - startFetchWait)
      context.taskMetrics.setShuffleTimeComplete(stopFetchWait)
      ...
      fetchUpToMaxBytes(true)
    }

    val nextDuration = System.currentTimeMillis() - startTime
    //logTrace(s"====Call for next to read blocks ${result.blockId} takes ${nextDuration} ms, done ${nextCount.getAndIncrement()} next")
    //currentResult = result.asInstanceOf[SuccessFetchResult]
    (result.blockId, new BufferReleasingInputStream(input, this))
  }

  private def fetchUpToMaxBytes(fromNext: Boolean = false): Unit = {
    ...
    if (deferredFetchRequests.nonEmpty) {
      for ((remoteAddress, defReqQueue) <- deferredFetchRequests) {
        logInfo(s"====deferredFetchRequests.nonEmpty is ${deferredFetchRequests.size}, to ${remoteAddress}, with ${defReqQueue.size}")
...
object ShuffleBlockFetcherIterator {
   ...
   * @param isNetworkReqDone Is this the last network request for this host in this fetch request.
   */
  private[storage] case class SuccessFetchChunkResult(
      blockId: BlockId,
      address: BlockManagerId,
      size: Long,
