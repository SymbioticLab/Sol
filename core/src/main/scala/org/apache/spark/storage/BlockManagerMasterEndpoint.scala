...
class BlockManagerMasterEndpoint(
  ...
  private var masterBlockManager: BlockManager = null
  ...
  override def receiveAndReply(context: RpcCallContext): PartialFunction[Any, Unit] = {
    ...
    case GetBlock(blockId) =>
      context.reply(getBlock(blockId))
  ...
  def registerBlockManager(blockManager: BlockManager): Unit = {
    masterBlockManager = blockManager
    logInfo(s"====Successfully register blockManager")
  }
  ...
  private def getBlock(blockId: BlockId): Array[Byte] = {
    assert (masterBlockManager != null)
    //val metadata = JavaUtils.bufferToArray(serializer.newInstance().serialize(("0")))
    val data:ManagedBuffer = masterBlockManager.getBlockData(blockId)
    logInfo(s"====getBlock ${blockId.name} has size ${data.size}")
    val array = JavaUtils.bufferToArray(data.nioByteBuffer())
    array
    //new UploadBlock("0", "0", blockId.name, metadata, array).toByteBuffer
  }
