...
private[spark] class TaskDescription(
    ...
    var attemptOrder: Long = 0,
    val shuffleInfo: String = "",
...
private[spark] object TaskDescription {
  ...
  def encode(taskDescription: TaskDescription): ByteBuffer = {
    ...
    dataOut.writeLong(taskDescription.attemptOrder)
    dataOut.writeLong(taskDescription.priority)
    dataOut.writeInt(taskDescription.attemptNumber)
    dataOut.writeInt(taskDescription.partitionId)
    dataOut.writeUTF(taskDescription.shuffleInfo)
  ...
  def decode(byteBuffer: ByteBuffer): TaskDescription = {
    ...
    var attemptOrder = dataIn.readLong()
    val priority = dataIn.readLong()
    val attemptNumber = dataIn.readInt()
    val partitionId = dataIn.readInt()
    val shuffleInfo = dataIn.readUTF()
    ...
    new TaskDescription(taskId, attemptNumber, executorId, name, index, taskFiles, taskJars,
      properties, serializedTask, attemptOrder, shuffleInfo, partitionId, priority)
  }
}
