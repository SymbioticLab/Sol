...
private[spark] class TaskSchedulerImpl(
  ...
  val pcEnable = conf.getBoolean("spark.pipelining.enabled", false)
  val pcDegree = conf.getInt("spark.pipelining.degree", 10)
  val proactiveBalancingEnable = conf.getBoolean("spark.proactiveBalancingEnable.enabled", false)
  val PRE_POST_ENABLE = conf.getBoolean("spark.prepost.enable", false)
  val PRE_POST_LATE_BINDING = conf.getBoolean("spark.prepostlate.enabled", false)
  ...
  // Total number of possible executors 
  val NUM_OF_EXECUTORS = math.ceil(conf.getInt("spark.cores.max", 1).toDouble/conf.getInt("spark.executor.cores", 1).toDouble).toInt
  ...
  override def submitTasks(taskSet: TaskSet) {
    ...
    val shuffleId = dagScheduler.stageIdToShuffleId.getOrElse(stageId, -1)

    logInfo("Adding task set " + taskSet.id + " with " + tasks.length + " tasks, shuffleId: " + shuffleId)
    dagScheduler.tasksToSchedule.put(stageId, tasks.length)
    mapOutputTracker.taskLocations.put(shuffleId, new Array[Int](NUM_OF_EXECUTORS))
  ...
  private def resourceOfferSingleTaskSet(
    ...
    var numOfTasksPlaced = 0
    val shuffleId = taskSet.shuffleId
    val stageId = taskSet.stageId
    val tasklocations = mapOutputTracker.taskLocations.get(shuffleId).orNull
    //logInfo(s"====Get taskLocations for shuffle: ${shuffleId}")
      ...
      if (availableCpus(i) >= CPUS_PER_TASK) {
        try {
          ...
          for (task <- taskSet.resourceOffer(execId, host, maxLocality)) {
            ...
            numOfTasksPlaced += 1
            // Must have already registered the locations
            tasklocations(execId.toInt) = tasklocations(execId.toInt) + 1
    ...
    dagScheduler.tasksToSchedule.put(stageId, dagScheduler.tasksToSchedule.get(stageId) - numOfTasksPlaced)
  ...
  def resourceOffers(offers: IndexedSeq[WorkerOffer]): Seq[Seq[TaskDescription]] = synchronized {
    ...
    //val shuffledOffers = filteredOffers
    // Build a list of tasks to assign to each worker.
    val tasks = shuffledOffers.map(o => new ArrayBuffer[TaskDescription](o.cores / CPUS_PER_TASK))
    var availableCpus: Array[Int] = null

    if (pcEnable && !proactiveBalancingEnable)
      availableCpus = shuffledOffers.map(o => o.cores + dagScheduler.ExecutorPcDegree.getOrElse(o.executorId, 0)).toArray
    else {
      availableCpus = shuffledOffers.map(o => o.cores).toArray
    }

    var totalAvailableCores = availableCpus.sum
    val scheduledStageIds = new ArrayBuffer[Int]

    if (totalAvailableCores > 0) {
      // TODO: to revive all stages in the rescheduling
      ...
      //logInfo(s"====Start to allocate taskset....")
      for (taskSet <- sortedTaskSets) {
        var launchedAnyTask = false
        var launchedTaskAtCurrentMaxLocality = false
        //logInfo(s"====Scan through taskSet ${taskSet.priority}")

        // first round for the real resource
        ...
        if (!launchedAnyTask) {
          ...
          scheduledStageIds += taskSet.stageId
        }

        // Might be a chance that we finish this taskSet
        // If so, we will add its child stages for pre-post
        if (PRE_POST_ENABLE && dagScheduler.tasksToSchedule.get(taskSet.stageId) == 0) {
          logInfo(s"====In prepost: tasksToSchedule == 0 for ${taskSet.stageId}")
          dagScheduler.tasksToSchedule.put(taskSet.stageId, -1)
          dagScheduler.prePostChild(taskSet.stageId)
        }
      }

      // if availableCpus is non-zero, then we pre-post child stages
      if (totalAvailableCores > 0 && PRE_POST_LATE_BINDING) {
        val _sortedTaskSets = rootPool.getSortedTaskSetQueue

        for (taskSet <- _sortedTaskSets) {
          var launchedAnyTask = false
          var launchedTaskAtCurrentMaxLocality = false
          // first round for the real resource
          for (currentMaxLocality <- taskSet.myLocalityLevels) {
