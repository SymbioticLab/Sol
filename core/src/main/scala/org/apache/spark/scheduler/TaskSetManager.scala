...
private[spark] class TaskSetManager(
  ...
  val subTime = clock.getTimeMillis()
  val shuffleInfo = sched.dagScheduler.stageShuffleInfo.getOrElse(stageId, "")
  var shuffleId = -1

  // MapShuffle Task
  if (shuffleInfo.length > 0 && shuffleInfo.contains("@")) {
    shuffleId = shuffleInfo.split("@")(0).split("_")(0).toInt
  }
  ...
  private[spark] def addPendingTask(index: Int, rerun: Boolean = false) {
    for (loc <- tasks(index).preferredLocations) {
      ...
      if (rerun) {
        logInfo(s"====Find a task ${index} in stage ${stageId} to rerun with preference ${loc}, now append to the list ....")
      }
  ...
  private def dequeueTaskFromList(
    ...
    while (indexOffset > 0) {
      ...
      if (!isTaskBlacklistedOnExecOrNode(index, execId, host)) {
        ...
        logInfo(s"====Task ${index} in stage ${stageId} is in the blacklist of host ${host}")
      }
    }
    None
  }
    ...
    if (!isZombie && !offerBlacklisted) {
      ...
      dequeueTask(execId, host, allowedLocality).map { case ((index, taskLocality, speculative)) =>
        ...
        logInfo(s"Starting $taskName (TID $taskId, $host, executor ${info.executorId}, " +
          s"partition ${task.partitionId}, $taskLocality, ${task.preferredLocations}, ${serializedTask.limit()} bytes), with allowedLocality ${allowedLocality}, and maxLocality ${maxLocality}")
        val _stageId = Math.floor(stageId.toFloat).toLong
        sched.dagScheduler.taskStarted(task, info)

        val partitionIndex = task match {
          case _task:ShuffleMapTask => _task.partitionIndex
          case _task => _task.partitionId
        }
  ...
  def handleTaskGettingResult(tid: Long): Unit = {
    ...
    if (sched.dagScheduler.resultGettingTime.containsKey(tid))
      info.markGettingResult(sched.dagScheduler.resultGettingTime.get(tid))
    else
  ...
  def handleFailedTask(tid: Long, state: TaskState, reason: TaskFailedReason) {
    ...
    if (info.failed || info.killed) {
      logInfo(s"====Looks weird where info.failed || killed for task ${tid} with ${reason}")
      return
    }
    removeRunningTask(tid)
    ...
    if (!isZombie && reason.countTowardsTaskFailures && !TOLERANCE_TEST) {
    ...
    if (successful(index)) {
      ...
      if (TOLERANCE_TEST) {
        addPendingTask(index, rerun = true)
        currentLocalityIndex = 0
        logInfo(s"====Due to task failures, now add pending task ${index} for stage ${stageId}")
      } else {
