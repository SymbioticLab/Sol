...
private[spark] class CompressedMapStatus(
    ...
    private[this] var sumSize: Long)
  extends MapStatus with Externalizable {

  protected def this() = this(null, null.asInstanceOf[Array[Byte]], -1)  // For deserialization only

  def this(loc: BlockManagerId, uncompressedSizes: Array[Long]) {
    this(loc, uncompressedSizes.map(MapStatus.compressSize), uncompressedSizes.sum)
  }
  ...
  override def getId(reduceId: Int): Int = {
    reduceId%compressedSizes.length.toInt
  }

  override def getSize: Long = sumSize

  override def numPartitions: Int = compressedSizes.size
...
private[spark] class HighlyCompressedMapStatus private (
    ...
    private[this] var sumSize: Long,
    private[this] var numBlocks: Long)
  extends MapStatus with Externalizable {

  // loc could be null when the default constructor is called during deserialization
  require(loc == null || avgSize > 0 || hugeBlockSizes.size > 0 || numNonEmptyBlocks == 0 || sumSize >= 0,
    "Average size can only be zero for map stages that produced no output")

  protected def this() = this(null, -1, null, -1, null, -1, -1)  // For deserialization only
  ...
  override def getId(reduceId: Int): Int = {
    reduceId%numBlocks.toInt
  }

  override def getSize: Long = sumSize

  override def numPartitions: Int = numBlocks.toInt
...
private[spark] object HighlyCompressedMapStatus {
  def apply(loc: BlockManagerId, uncompressedSizes: Array[Long]): HighlyCompressedMapStatus = {
    ...
    new HighlyCompressedMapStatus(loc, numNonEmptyBlocks, emptyBlocks, avgSize,
      hugeBlockSizesArray.toMap, totalSize, totalNumBlocks)
  }
}
