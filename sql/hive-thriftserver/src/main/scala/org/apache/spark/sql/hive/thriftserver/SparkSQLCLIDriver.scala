  ...
  def main(args: Array[String]) {
    ...
    try {
      if (sessionState.fileName != null) {
        logInfo("====== SessionState.fileName: " + sessionState.fileName)
        val sqlSets = sessionState.fileName.split("__")
        val executors=Executors.newFixedThreadPool(sqlSets.size)
        var readIndicator = false
        var arrivals = "".split("\t")
        var cnt = 0

        // read the info about arrivals
        if (Files.exists(Paths.get("/users/jimmyyou/hive-testbench/arrival.cfg"))){
          for (line <- Source.fromFile("/users/jimmyyou/hive-testbench/arrival.cfg").getLines){
            arrivals = line.split("\t")
          }
        }

        if (arrivals.size == sqlSets.size && arrivals(0) != "")
          readIndicator = true

        for (i <- 0 until sqlSets.size){
          logInfo("====== Submit: " + sqlSets(i))
          if (readIndicator == true){
            executors.submit(new OnlineSqlRunnable(cli, sqlSets(i), arrivals(i).toLong))
          }
          else{
            Thread.sleep(500)
            executors.submit(new SqlRunnable(cli, sqlSets(i)))
          }
        }

        executors.shutdown()
        logInfo("Have submitted all sqls: " + sqlSets.size)

        var loop = true
        while (loop){
          Thread.sleep(500)
          loop = !executors.isTerminated()
        }

        logInfo("======== Done all sqls =======")
...
class SqlRunnable(cli: SparkSQLCLIDriver, fileName: String) extends Runnable {
  override def run(): Unit = {
    cli.processFile(fileName)
  }
}

class OnlineSqlRunnable(cli: SparkSQLCLIDriver, fileName: String, arrival: Long) extends Runnable {
  override def run(): Unit = {
    if (arrival > 0L)
