...
/**
  ...
  public void fetchChunk(
    ...
    channel.writeAndFlush(new ChunkFetchRequest(streamChunkId)).addListener(future -> {
      if (future.isSuccess()) {
        ...
        logger.trace("Sending Fetch request {} to {} took {} ms", streamChunkId,
  ...
  public void fetchChunk(
    ...
    StreamChunkId streamChunkId = new StreamChunkId(streamId, chunkIndex, priority, System.currentTimeMillis());
    handler.addFetchRequest(streamChunkId, callback);
    ...
    channel.writeAndFlush(new ChunkFetchRequest(streamChunkId)).addListener(future -> {
      if (future.isSuccess()) {
        long timeTaken = System.currentTimeMillis() - startTime;
        //if (logger.isTraceEnabled()) {
        logger.info("Sending request {}, priority {}, from {} to {} took {} ms", streamChunkId, priority,
            channel.localAddress() , getRemoteAddress(channel), timeTaken);
        //}
      } else {
        String errorMsg = String.format("Failed to send request %s to %s: %s", streamChunkId,
          getRemoteAddress(channel), future.cause());
    ...
    channel.writeAndFlush(new RpcRequest(requestId, new NioManagedBuffer(message)))
        .addListener(future -> {
          if (future.isSuccess()) {
            long timeTaken = System.currentTimeMillis() - startTime;
            //if (logger.isTraceEnabled()) {
              logger.info("Sending Rpcrequest {} from {} to {} took {} ms", requestId, channel.localAddress(),
                getRemoteAddress(channel), timeTaken);
            //}
          } else {
